...
function Canceled() {
  ...
  // ---- Sound helpers (soft neutral chime) ----
  const ensureContext = () => {
  ...
  const playCancelTone = () => {
    const ac = ensureContext();
    if (!ac) return;
    const now = ac.currentTime + 0.02;

    const base = 659.25; // E5 calm tone
    const overtones = [
      { f: base, a: 0.3 },
      { f: base * 1.5, a: 0.18 },
    ];
    ...
    overtones.forEach(({ f, a }) => {
      ...
      g.gain.linearRampToValueAtTime(a, now + 0.02);
      g.gain.exponentialRampToValueAtTime(0.0001, now + 1.2);
      // tiny downward tail
      o.frequency.setValueAtTime(f * 1.02, now);
      o.frequency.exponentialRampToValueAtTime(f * 0.92, now + 0.1);
      o.start(now);
      o.stop(now + 1.3);
    });
  };
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundImage:
          "repeating-linear-gradient(135deg, rgba(255,255,255,.06) 0 2px, rgba(255,255,255,0) 2px 26px), radial-gradient(1200px 500px at 50% -300px, rgba(255,255,255,.14), transparent 60%), linear-gradient(180deg, #d8a300, #ba8a00)",
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            background: "radial-gradient(circle at 35% 35%, #fff, #fff8e1 70%)",
            boxShadow:
              "0 10px 26px rgba(0,0,0,.2), inset 0 0 0 6px rgba(255,193,7,.22)",
          }}
        >
          {/* pulse glow */}
          <span
            ...
            style={{
              ...
              boxShadow: "0 0 0 0 rgba(255,193,7,.34)",
            }}
          />
          {/* Minus Symbol (Circle Slash) */}
          <svg
            className="block"
        ...
        <h1 className="text-[clamp(20px,6vw,30px)] leading-tight mb-1 text-amber-700">
          Reservation Canceled
        </h1>
        <p className="text-slate-600 text-[clamp(13px,4.5vw,17px)] leading-relaxed m-0">
          Your booking was canceled. If this was a mistake, please book again.
        </p>
        <motion.p
      ...
      {showSoundPill && (
        <button
          onClick={tryAutoplay}
          className="fixed left-1/2 -translate-x-1/2 font-bold text-white rounded-full px-4 py-2 shadow-[0_10px_22px_rgba(186,138,0,.35)]"
          style={{
            bottom: "max(12px, env(safe-area-inset-bottom))",
            background: "linear-gradient(180deg, #f0b400, #c69300)",
      ...
      <style>
        ...
        @keyframes pulse { 0%{box-shadow:0 0 0 0 rgba(255,193,7,.34)} 100%{box-shadow:0 0 0 20px rgba(255,193,7,0)} }
        @keyframes draw { to { stroke-dashoffset: 0; } }
        @keyframes glow { 50% { filter: drop-shadow(0 0 10px rgba(186,138,0,.8)); } 100% { filter: none; } }
      `}
      </style>
    </div>
  );
